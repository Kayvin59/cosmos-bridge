  ...
  return (
    <Box>
      ...
      <Stack attributes={{ alignItems: 'center' }}>
        <Box mr="$6">
          <Image width="32" height="32" alt={from.symbol} src={from.logo ?? ''} />
        </Box>
        <Dashes />
        {steps.map((route: SwapRouteStep) => (
          <Fragment key={route.poolId}>
            <Box ml="$6" mr="$5" width="$16" height="$12" position="relative">
              <Image width="32" height="32" alt={route.base.symbol} src={route.base.logo ?? ''} />
              <Box top="$0" right="$0" position="absolute">
                <Image width="32" height="32" alt={route.quote.symbol} src={route.quote.logo ?? ''} />
              </Box>
            </Box>
        ...
        <Box ml="$6">
          <Image width="32" height="32" alt={to.symbol} src={to.logo ?? ''} />
        </Box>
      </Stack>
    </Box>
  );
